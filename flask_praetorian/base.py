        ...
        for place in self.jwt_places:
            try:
                ...
                flask.current_app.logger.warning(
                    "Flask_Praetorian hasn't implemented reading JWT tokens from location {place}. Please reconfigure JWT_PLACES.".format(  # noqa: E501
                        place=place.lower(),
                        )
                    )
                flask.current_app.logger.warning(
                    "Values accepted in JWT_PLACES are: {def_places}".format(
                        def_places=DEFAULT_JWT_PLACES,
                        )
                    )

        raise MissingToken(
            "Could not find token in any of the given locations: {locations}".format(
