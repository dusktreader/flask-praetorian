...
class TestPraetorian:
    ...
    def test_encode_jwt_token(self, app, user_class, validating_user_class):
        ...
        moment = datetime.datetime(
            2017, 5, 21, 18, 39, 55, tzinfo=datetime.timezone.utc
        )
        with freezegun.freeze_time(moment):
            ...
            assert token_data["iat"] == int(moment.timestamp())
            assert token_data["exp"] == int(
                (moment + DEFAULT_JWT_ACCESS_LIFESPAN).timestamp()
            )
            assert token_data[REFRESH_EXPIRATION_CLAIM] == int(
                (moment + DEFAULT_JWT_REFRESH_LIFESPAN).timestamp()
            )
        ...
        override_access_lifespan = datetime.timedelta(minutes=1)
        override_refresh_lifespan = datetime.timedelta(hours=1)
        with freezegun.freeze_time(moment):
            ...
            assert token_data["iat"] == int(moment.timestamp())
            assert token_data["exp"] == int(
                (moment + override_access_lifespan).timestamp()
            )
            assert token_data[REFRESH_EXPIRATION_CLAIM] == int(
                (moment + override_refresh_lifespan).timestamp()
            )
            assert token_data["id"] == the_dude.id
            assert token_data["rls"] == "admin,operator"
        ...
        override_access_lifespan = datetime.timedelta(hours=1)
        override_refresh_lifespan = datetime.timedelta(minutes=1)
        with freezegun.freeze_time(moment):
            token = guard.encode_jwt_token(
                the_dude,
                override_access_lifespan=override_access_lifespan,
        ...
        moment = datetime.datetime(
            2018, 8, 18, 8, 55, 12, tzinfo=datetime.timezone.utc
        )
        with freezegun.freeze_time(moment):
            token = guard.encode_jwt_token(
                the_dude,
    ...
    def test_encode_eternal_jwt_token(self, app, user_class):
        ...
        with freezegun.freeze_time(moment):
            ...
            assert token_data["iat"] == int(moment.timestamp())
            assert token_data["exp"] == int(
                (moment + VITAM_AETERNUM).timestamp()
            )
            assert token_data[REFRESH_EXPIRATION_CLAIM] == int(
                (moment + VITAM_AETERNUM).timestamp()
            )
    ...
    def test_refresh_jwt_token(
        ...
        with freezegun.freeze_time(new_moment):
            ...
            assert new_token_data["iat"] == int(new_moment.timestamp())
            assert new_token_data["exp"] == int(
                (new_moment + DEFAULT_JWT_ACCESS_LIFESPAN).timestamp()
            )
            assert new_token_data[REFRESH_EXPIRATION_CLAIM] == int(
                (moment + DEFAULT_JWT_REFRESH_LIFESPAN).timestamp()
            )
        ...
        with freezegun.freeze_time(new_moment):
            new_token = guard.refresh_jwt_token(
                token,
                override_access_lifespan=datetime.timedelta(hours=2),
            )
            new_token_data = jwt.decode(
                new_token,
                guard.encode_key,
            ...
            assert new_token_data["exp"] == int(
                (new_moment + datetime.timedelta(hours=2)).timestamp()
            )
        ...
        with freezegun.freeze_time(moment):
            token = guard.encode_jwt_token(
                the_dude,
                override_refresh_lifespan=datetime.timedelta(hours=2),
                override_access_lifespan=datetime.timedelta(minutes=30),
            )
        new_moment = moment + datetime.timedelta(minutes=31)
        with freezegun.freeze_time(new_moment):
            new_token = guard.refresh_jwt_token(
                token,
                override_access_lifespan=datetime.timedelta(hours=2),
        ...
        expiring_interval = (
            DEFAULT_JWT_ACCESS_LIFESPAN.as_timedelta()
            + datetime.timedelta(minutes=1)
        )
        validating_guard = Praetorian(app, validating_user_class)
        brandt = validating_user_class(
        ...
        moment = datetime.datetime(
            2018, 8, 14, 9, 5, 24, tzinfo=datetime.timezone.utc
        )
        with freezegun.freeze_time(moment):
            token = guard.encode_jwt_token(
                the_dude,
    ...
    def test_pack_header_for_user(self, app, user_class):
        ...
        moment = datetime.datetime(
            2018, 8, 14, 9, 8, 39, tzinfo=datetime.timezone.utc
        )
        with freezegun.freeze_time(moment):
            header_dict = guard.pack_header_for_user(
                the_dude,
    ...
    def test_get_user_from_registration_token(
        ...
        moment = datetime.datetime(
            2019, 1, 30, 16, 40, tzinfo=datetime.timezone.utc
        )
        with freezegun.freeze_time(moment):
            with pytest.raises(ExpiredAccessError):
